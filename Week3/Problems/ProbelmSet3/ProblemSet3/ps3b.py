...
class SimpleVirus(object):
    ...
    def doesClear(self):
        ...
        False.
        """
        chanceIsCleared = random.random()
        return chanceIsCleared < self.getClearProb()
...
class Patient(object):
    ...
    def update(self):
        ...
        for it in list(reversed(range(len(toUpdate)))):
            if toUpdate[it]:
                self.viruses.pop(it)
        # Calculamos la densisdad de viruses restantes
        self.totalPop = len(self.viruses)
        # Si procede, cada virus se reproduce
        for i in range(self.totalPop):
            try:
                popDensity = float(self.getTotalPop()/float(self.getMaxPop()))
                newVirus = self.viruses[i].reproduce(popDensity)
...
def initPatient(numViruses, maxPop, maxBirthProb, clearProb):
    viruses = []
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    timeSteps = 300
    avarageVirPop = [0 for i in range(timeSteps)]
    for i in range(numTrials):
        patient = initPatient(numViruses, maxPop, maxBirthProb, clearProb)
        for ts in range(timeSteps):
            virPop = patient.update()
            avarageVirPop[ts] += virPop
    for i in range(len(avarageVirPop)):
        avarageVirPop[i] = avarageVirPop[i]/float(numTrials)
    pylab.plot(avarageVirPop, label='avarage virus population')
    pylab.title('Evolution of the virus population during time')
    pylab.xlabel('Time steps')
    pylab.ylabel('Avarage virus population')
    pylab.legend(loc='best', framealpha=0.5, prop={'size': 'small'})
    pylab.show()
...
class ResistantVirus(SimpleVirus):
    ...
    def isResistantTo(self, drug):
        ...
        is called by getResistPop() in TreatedPatient to determine how many vir
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        A virus particle will only reproduce if it is resistant to ALL the drug
        ...
        if chanceReproduce < self.maxBirthProb * (1 - popDensity):
            ...
            newVirus = ResistantVirus(self.getMaxBirthProb(),
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        for virus in self.getViruses():
            virus.getResistances().update({newDrug: True})
        return
...
class ResistantVirus(SimpleVirus):
    ...
    def __init__(self, maxBirthProb, clearProb, resistances, mutProb):
        ...
        resistances: A dictionary of drug names (strings) mapping to the state
        of this virus particle's resistancre (either True or False) to each drug
        e.g. {'guttagonol':False, 'srinol':False}, means that this virus
        particle is resistant to neither guttagonol nor srinol.
...
# PROBLEM 5
#
def simulationWithDrug(numViruses, maxPop, maxBirthProb,
    ...
    viruses = [ResistantVirus(maxBirthProb, clearProb, resistances, mutProb)
               for i in range(numViruses)]
    virPop = [0 for i in range(150)]
    virPopWithDrugs = [0 for i in range(150)]
    for i in range(numTrials):
        p = TreatedPatient(viruses, maxPop)
        for i in range(150):
            pop = p.update()
            virPop[i] += pop
        p.addPrescription('guttagnol')
        for i in range(150):
            pop = p.update()
            virPopWithDrugs[i] += pop
    for i in range(150):
        virPop[i] /= float(numTrials)
        virPopWithDrugs[i] /= float(numTrials)

    # Ploting
    pylab.plot(virPop, label='avarage virus population without drugs')
    pylab.plot(virPopWithDrugs, label='avarage virus population with drugs')
    pylab.title('Evolution of the virus in a patient with and without drugs')
    pylab.xlabel('Time steps')
    pylab.ylabel('Avarage virus population')
    pylab.legend(loc='best', framealpha=0.5, prop={'size': 'small'})
    pylab.show()

...
class Cluster(object):
    ...
    def averageLinkageDist(self, other):
        ...
        for p in self.points:
            for otherP in other.points:
                distances.append(p.distance(otherP))
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        self.members.append(Cluster(newPoints, type(newPoints[0])))
    ...
    def findClosest(self, linkage):
        ...
        bestPair = ()
        bestDist = 99999999999999
        mem = self.getClusters()
        indice = 0
        print 'indice', indice
        while indice < len(mem)-1:
            print 'indice: ', indice, ' indice-1', indice-1
            dist = linkage(mem[indice], mem[indice+1])
            if dist < bestDist:
                bestPair = (mem[indice], mem[indice+1])
                bestDist = dist
            indice += 1
        return bestPair
    ...
    def mergeOne(self, linkage):
        ...
        cc = self.findClosest(linkage)
        self.mergeClusters(cc[0], cc[1])
        return cc
...
cs = ClusterSet(type(a))
for i in points:
    cs.add(Cluster(i, type(a)))

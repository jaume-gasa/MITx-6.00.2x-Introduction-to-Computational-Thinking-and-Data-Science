    ...
    for r in rollList:
        if not lastR == '':
            if lastR == r:
                ...
                maxRunsList.append(maxRuns)
                maxRuns = 1
        lastR = r
    maxRunsList.append(maxRuns)
    return max(maxRunsList)
...
def getAverage(die, numRolls, numTrials):
    ...
    for i in range(numTrials):
        run = getRuns(die, numRolls)
        allRuns.append(run)
    makeHistogram(allRuns, 10, 'Value of the dice', 'Avarage of runs', 'Avarage of runs trhough time')
    a, b = getMeanAndStd(allRuns)
    return a

# print getRuns(Die([1, 2, 3, 4, 5, 6, 6, 6, 7]), 10)
# One test case
